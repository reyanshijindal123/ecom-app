...
export default function ProductDetailPage() {
  ...
  if (isError || !product) {
    return (
      <div className="text-center py-24">
        ...
        <Link
          href="/products"
          className="mt-4 inline-block text-[#970747] underline text-sm"
        >
          ← Back to products
        </Link>
      </div>
  ...
  const discount = Math.round(
    ((product.price * 1.35 - product.price) / (product.price * 1.35)) * 100,
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-8">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-400 mb-6">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-1 hover:text-[#970747] transition-colors"
        >
          <ArrowLeft size={13} /> Back
        </button>
        <span>/</span>
        <Link href="/products" className="hover:text-[#970747]">
          Products
        </Link>
        <span>/</span>
        <span className="text-gray-500 capitalize truncate max-w-[120px]">
          {product.category}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-10 mb-12">
        ...
        <div className="space-y-3">
          <div
            className="bg-white rounded-3xl p-8 flex items-center justify-center border border-gray-100 shadow-sm relative overflow-hidden"
            style={{ minHeight: 320 }}
          >
            ...
            <div className="absolute top-3 right-3 flex gap-2">
              <button
                onClick={handleShare}
                className="w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-gray-500 hover:text-[#970747] transition-colors"
              >
                <Share2 size={14} />
              </button>
          ...
          {sizes.length > 0 && (
            <div>
              <p className="text-sm font-bold text-gray-700 mb-2">
                Size:{" "}
                {selectedSize ? (
                  <span className="text-[#970747]">{selectedSize}</span>
                ) : (
      ...
      setSelectedSize(s);
          ...
          <div className="space-y-3">
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl font-black text-sm transition-all duration-300 ${
                  added
                    ...
                    : "bg-[#970747] hover:bg-[#7a0538] text-white shadow-lg shadow-[#970747]/20 hover:-translate-y-0.5"
                }`}
              >
                {added ? (
                  <>
            ...
            {added && (
              <Link
                href="/cart"
                className="block text-center text-sm text-[#970747] font-semibold hover:underline"
              >
                → View Cart
              </Link>
            )}
          ...
          <div className="space-y-2.5 border-t border-gray-100 pt-4">
            <div className="flex items-center gap-2.5 text-sm text-gray-600">
              ...
              <span>
                Estimated delivery:{" "}
                <strong className="text-gray-800">{getDeliveryDate(4)}</strong>
              </span>
            </div>
            <div className="flex items-center gap-2.5 text-sm text-gray-600">
      ...
      {related && related.length > 0 && (
        <div className="mt-12">
          <h2 className="text-xl font-black text-gray-900 mb-5">
            More in{" "}
            <span className="text-[#970747] capitalize">
              {product.category}
            </span>
          </h2>
