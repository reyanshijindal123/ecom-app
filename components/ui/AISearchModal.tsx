...
Respond with a JSON object only (no markdown):
{
  ...
  "suggestion": "1-2 sentence friendly response about what you found/recommended"
      ...
      setSuggestion(
        "I found some great options! Let me show you the products.",
      );
      setSearch(query);
    } finally {
      setLoading(false);
      ...
      <div className="space-y-4">
        <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-xl p-3 flex items-start gap-3">
          ...
          <p className="text-sm text-gray-600">
            Describe what you&apos;re looking for in natural language and
        ...
        <div className="relative">
          <textarea
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && e.ctrlKey) handleAISearch();
            }}
          />
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              key={s}
              onClick={() => setQuery(s)}
              className="text-xs px-3 py-1.5 bg-pink-50 text-[#970747] rounded-full hover:bg-pink-100 transition-colors"
            >
              {s}
            </button>
          ))}
