      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {productReviews.length > 0 && (
          <div className="flex items-center gap-2">
            <div className="flex">{[1,2,3,4,5].map(s => <Star key={s} size={14} className={s <= Math.round(avgRating) ? 'fill-amber-400 text-amber-400' : 'text-gray-200 fill-gray-200'} />)}</div>
        ...
        <div className="flex gap-1 mb-3">
          {[1,2,3,4,5].map(s => (
            <button key={s} onClick={() => setRating(s)} onMouseEnter={() => setHovered(s)} onMouseLeave={() => setHovered(0)}>
              <Star size={22} className={`transition-colors ${s <= (hovered || rating) ? 'fill-amber-400 text-amber-400' : 'text-gray-300 fill-gray-100'}`} />
            </button>
          ))}
        </div>
        <textarea
          ...
          placeholder={isAuthenticated ? 'Share your experience with this product...' : 'Login to write a review'}
          ...
          {productReviews.map(r => (
            <div key={r.id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex">{[1,2,3,4,5].map(s => <Star key={s} size={11} className={s <= r.rating ? 'fill-amber-400 text-amber-400' : 'text-gray-200 fill-gray-200'} />)}</div>
              </div>
